...
def evaluateModel(model):
    ...
    for i in tqdm(range(10)):
        ...
        accury = [ (x==y) for x,y in zip(new_x,y_test2) ]
	acc = np.mean(accury)
        print" Acc = ", acc
        #
   ...
   # model.load(best_model_path)
    ...
    #print(prediction.shape)
    print(pd.crosstab(y_test_categories,prediction,rownames=['label'],colnames=['predict']))
    #print(model.metrics," xxx ",model.evaluate(x_test,y_test_categories))

    #print( 'Accuracy_best = ', model.evaluate(x_test, y_test)[1])
...
if __name__ == "__main__":
    ...
    best_model_path = '../models/best_model_my1_' + models[n-1] + '.h5'
